...
def compute_attack_vector_probabilities(df):
    # Parse attack vector from cvss_vectorstring
    def extract_av(v):
        try:
            for part in v.split("/"):
    ...
    df["attack_vector"] = df["cvss_vectorstring"].astype(str).apply(extract_av)

    # Basic probability table
    prob_table = (
        df.groupby("attack_vector")
          .agg(total=("is_kev", "size"),
               kev_count=("is_kev", "sum"))
          .reset_index()
    )
    prob_table["P_KEV_given_AV_pct"] = (
        prob_table["kev_count"] / prob_table["total"] * 100
    ).round(3)

    return prob_table
def make_attack_vector_probability_chart(prob_table):
    fig = px.bar(
        prob_table,
        x="attack_vector",
        y="P_KEV_given_AV_pct",
        title="Probability of KEV by Attack Vector",
        text="P_KEV_given_AV_pct"
    )

    fig.update_traces(texttemplate='%{text:.2f}%', textposition='outside')
    fig.update_layout(yaxis_title="P(KEV | Attack Vector) (%)")

    return fig


def extract_attack_vector(vec):
    if not isinstance(vec, str) or "AV:" not in vec:
        return None
    try:
        code = vec.split("AV:")[1].split("/")[0]
        return AV_MAP.get(code, None)
    except:
        return None
...
df["attack_vector"] = df[VECTOR_COL].apply(extract_attack_vector)
...
def update_charts(year_range, severities, kev_mode, score_pair, hazard_mode):
    ...
    prob_table = compute_attack_vector_probabilities(dff)
    fig_attack_probs = make_attack_vector_probability_chart(prob_table)

    clean_av = dff.dropna(subset=["attack_vector", "is_kev"])
    av_stats = (
        clean_av
    ...
    # ---- NEW CLEAN PROBABILITY TABLE + BAR CHART ----
    prob_table = compute_attack_vector_probabilities(dff)
    fig_attack_probs = make_attack_vector_probability_chart(prob_table)

    prob_table_display = prob_table.to_dict("records")
